...
public class ServicoPersist : IServicoPersist
{
    ...
    public async Task<Servico[]> GetAllServicosAtivosByPetShopIdAsync(int petShopId, bool ativo) => await _context.Servicos.Where(p => p.PetShopId == petShopId && p.Ativo == ativo)
                                                                                                                           .OrderBy(p => p.Nome)
                                                                                                                           .ToArrayAsync();

    public async Task<Servico> GetServicoByIdAsync(int servicoId)
    {
        return await _context.Servicos.Where(p => p.Id == servicoId).FirstOrDefaultAsync();
    }
}
